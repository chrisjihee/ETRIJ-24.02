...
\begin{document}
    ...
    In the BERT-based re-ranking described in Section~\ref{sec:reranking-model}, we compared the performances using three pre-trained language models: KPF-BERT, ETRI-ELECTRA, and ETRI-RoBERTa, known for their proficiency in other Korean language understanding tasks.
    The models were fine-tuned for re-ranking with distinct inputs.
    Preliminary tests indicated that using the same type of model or input yielded nearly identical retrained models, with no further improvement in performance.
