...
\begin{document}
    ...
    This is attributed to several factors that inherently increase the complexity and computational demand of the system.
    The inclusion of two-stage re-ranking model adds another layer of complexity.
    The re-ranking process requires the evaluation of all generated secondary paths per each sentence by the morphological analysis to determine the most plausible results.
    This step involves significant computational resources and require substantial processing power to make predictions.
    Furthermore, real-time processing demands instantaneous responses, typically within milliseconds, to ensure a seamless user experience.
    The latency introduced by the detailed analysis and re-ranking stages may exceed these latency requirements, leading to delays that are perceptible to the end-user.
    When operating in real-time, the system must handle a continuous stream of data, necessitating a high-throughput capability that can accommodate the swift processing of information without bottlenecks.
    The current model is not be designed to efficiently manage such high volumes of data at the speed required for real-time interactions.
